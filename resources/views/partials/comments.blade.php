...
<div class="panel panel-primary shadow">
    ...
    <div class="panel-body">
        ...
        <p class="smalltext">{{ __('Créé à') }}:
            {{ date('d F, Y, H:i:s', strtotime($subject->created_at))}}
            @if($subject->updated_at != $subject->created_at)
                <br/>{{ __('Modifié') }}: {{date('d F, Y, H:i:s', strtotime($subject->updated_at))}}
            @endif</p>
    </div>
</div>
...
@if($instance == 'task')
    ...
    <div class="form-group">
        ...
        {!! Form::submit( __('Ajouter un commentaire') , ['class' => 'btn btn-primary']) !!}
    </div>
    {!! Form::close() !!}
@else
